...
let emojiDataPath = path.resolve(__dirname, 'unicode_emoji_data');
...
const UNICODE_EMOJI_VERSION_STRING = UNICODE_VERSION.slice(0, 2).join('.');
 ...
 * @return {Promise<string>}
 */
let fetchEmojiData = async f => {
  let url = new URL(`https://www.unicode.org/Public/emoji/${UNICODE_EMOJI_VERSION_STRING}/${f}`);
  let filePath = path.join(emojiDataPath, f);
...
let [
  ...
  emojiData,
  emojiSequenceData,
  emojiZwjSequenceData,
  // wordData,
  // sentenceData,
] = await Promise.all([
  fetchData('auxiliary/GraphemeBreakProperty.txt'),
  fetchData('emoji/emoji-data.txt'),
  fetchEmojiData('emoji-sequences.txt'),
  fetchEmojiData('emoji-zwj-sequences.txt'),
